...
project's edge node:
```bash
kubectl get svc edge-foobar-12 -n $MY_NAMESPACE
```
...
```bash
kubectl create secret generic greymatter-foobar-12-edge-certs \
...
It is up to you to get them there! Following the pattern defined in the `k8s/manifests.yaml` edge-foobar-12 Deployment 
